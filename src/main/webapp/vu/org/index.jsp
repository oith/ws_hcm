...
<tiles:putAttribute name="body">
    ...
    <form:form action="${pageContext.request.contextPath}/org/index" commandName="searchCriteria" method="POST">
    ...
    <c:if test="${orgs.size()!=0}">
        <div style="margin: auto; overflow-x: scroll; padding-top: 5px">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="style-table">
                ...
                <tbody>
                    <c:forEach items="${orgs}" var="org"  varStatus="loopStatus">

                        <tr class="${loopStatus.index % 2 == 0 ? 'odd' : 'even'}">
                            <td><a href="${pageContext.request.contextPath}/org/show/<c:out value="${org.id}"/>"><spring:message code="show.link.label"/></a></td>
                            <td><c:out value="${org.code}"/></td>
                            <td><c:out value="${org.name}"/></td>
                            <td><c:out value="${org.description}"/></td>

                            <td><a href="${pageContext.request.contextPath}/org/edit/<c:out value="${org.id}"/>"><spring:message code="edit.link.label"/></a></td>
                            <td><a href="${pageContext.request.contextPath}/org/delete/<c:out value="${org.id}"/>" onclick="return confirm('Are you sure to delete?');" ><spring:message code="delete.link.label"/></a></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
