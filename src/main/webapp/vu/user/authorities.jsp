...
<spring:message code="authorities" text="User Role Details"/> | 
<button id="edu_crt" type="button" data-toggle="modal" data-target="#myModalRole"><spring:message code="create.link.label"/>&NonBreakingSpace;<spring:message code="Role" text="User Role"/></button>


<c:if test="${not empty user.authorities}">
    <div id="div_ProfileRoleDtls" style="margin: auto; overflow-x: scroll; padding-top: 10px">
...
<!--                    <td><spring:message code="passingYear" text="Passing Year"/></td>
            ...
            <tbody>
                <c:forEach items="${user.authorities}" var="role"  varStatus="loopStatus">

                    <tr  class="${loopStatus.index % 2 == 0 ? 'odd' : 'even'}">
                        <td class="not_show"><c:out value="${role.id}"/></td>
                        <td><button id="${user.id}~${role.id}" type="button" value="${user.id}~${role.id}" data-toggle="modal" data-target="#myModalRole"><spring:message code="edit.link.label" text="Edit"/></button></td>
    ...
    <div class="modal fade" id="myModalRole" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <form:form action="${pageContext.request.contextPath}/role/edit" enctype="multipart/form-data" commandName="role" method="POST">

                    <!--modal-body-->
                    <div class="modal-body">
                        <!--<div class="container">-->
                        <h2><spring:message code="Role" text="User Role"/></h2>

                        <div class="col-xs-6">
                            <div class="form-group">
                                <label>User ID</label>
                                <input type="text" name="userId" id="userId" class="form-control" value="${user.id}" >
                            </div>
                        </div>
                        ...
                        <div class="col-xs-6">
                            <div class="form-group">
                                <label>ID</label>
                                <input type="text" name="id" id="edu_id" class="form-control" value="" >
                            </div>
                        </div>
    ...
    $(document).on("click", ".edu_dtl_del", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                $("#div_ProfileRoleDtls").load(location.href + " #div_ProfileRoleDtls>*", "");
