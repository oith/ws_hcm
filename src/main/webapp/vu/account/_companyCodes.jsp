...
<c:if test='${not empty account.companyCodes}'>
    <div class='table-responsive'>
        <table class='table table-striped table-bordered table-hover table-condensed'>
            ...
            <tbody>
                <c:forEach items='${account.companyCodes}' var='companyCodes' varStatus='loopStatus'>
                    <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                        <td><c:out value='${loopStatus.index}'/></td>
                        <td><button id='${account.id}~${loopStatus.index}' type='button' class='btn btn-info' value='${account.id}~${loopStatus.index}' data-toggle='modal' data-target='#companyCodes_modal' class='btn btn-primary'><spring:message code='edit.link.label' text='Edit'/></button></td>

                        <td><button type='button' class='companyCodes_del btn btn-warning' value='companyCodes~${account.id}~${loopStatus.index}'><spring:message code='erase.button.label' text='Erase'/></button></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
    ...
    <div class='modal fade' id='companyCodes_modal' role='dialog'>
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form:form action='${pageContext.request.contextPath}/account/companyCodes/edit/${account.id}' commandName='companyCodes' method='POST' class='form-horizontal'>
