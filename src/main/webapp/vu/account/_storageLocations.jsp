...
<c:if test='${not empty account.storageLocations}'>
    <div class='table-responsive'>
        <table class='table table-striped table-bordered table-hover table-condensed'>
            ...
            <tbody>
                <c:forEach items='${account.storageLocations}' var='storageLocations' varStatus='loopStatus'>
                    <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                        <td><c:out value='${loopStatus.index}'/></td>
                        <td><button id='${account.id}~${loopStatus.index}' type='button' class='btn btn-info' value='${account.id}~${loopStatus.index}' data-toggle='modal' data-target='#storageLocations_modal' class='btn btn-primary'><spring:message code='edit.link.label' text='Edit'/></button></td>

                        <td><button type='button' class='storageLocations_del btn btn-warning' value='storageLocations~${account.id}~${loopStatus.index}'><spring:message code='erase.button.label' text='Erase'/></button></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
    ...
    <div class='modal fade' id='storageLocations_modal' role='dialog'>
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form:form action='${pageContext.request.contextPath}/account/storageLocations/edit/${account.id}' commandName='storageLocations' method='POST' class='form-horizontal'>
