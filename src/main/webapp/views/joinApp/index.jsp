...
<tiles:putAttribute name='body'>
    ...
    <form:form action='${pageContext.request.contextPath}/joinApp/index' commandName='searchCriteria' method='POST'>
        <table>
            <tr>
            ...
            <table class='table table-striped table-bordered table-hover table-condensed'>
                <thead>
                    <tr>
                        ...
                        <th><spring:message code='position' text='Position'/></th>
                        <th><spring:message code='approval' text='Approval'/></th>

                    </tr>
                </thead>
                <tbody>
                    <c:forEach items='${joinApps}' var='joinApp'  varStatus='loopStatus'>
                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                            <td>
                                <a href="${pageContext.request.contextPath}/joinApp/show/${joinApp.id}" class="btn btn-info">
                                    <i class="glyphicon glyphicon-book"></i>
                                    <spring:message code="show.link.label"/>
                                </a>
                            </td>
                            <td><c:out value='${joinApp.auditor}'/></td>
                            <td><c:out value='${joinApp.profile}'/></td>
                            <td><c:out value='${joinApp.code}'/></td>
                            <td><fmt:formatDate value='${joinApp.appDate}' type='date' pattern='dd/MM/yyyy'/></td>
                            <td><c:out value='${joinApp.remarks}'/></td>
                            <td><c:out value='${joinApp.interval}'/></td>
                            <td><c:out value='${joinApp.position}'/></td>
                            <td><c:out value='${joinApp.approval}'/></td>
                            ...
                            <td>
                                <a href="${pageContext.request.contextPath}/joinApp/delete/${joinApp.id}" class="btn btn-danger" onclick="return confirm('Are you sure to delete?');" >
