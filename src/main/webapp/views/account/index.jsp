...
<tiles:putAttribute name='body'>
    ...
    <form:form action='${pageContext.request.contextPath}/account/index' commandName='searchCriteria' method='POST'>
        <table>
            <tr>
            ...
            <table class='table table-striped table-bordered table-hover table-condensed'>
                <thead>
                    <tr>
                        ...
                        <th><spring:message code='accountStatement' text='Account Statement'/></th>
                ...
                <tbody>
                    <c:forEach items='${accounts}' var='account'  varStatus='loopStatus'>
                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                            <td>
                                <a href="${pageContext.request.contextPath}/account/show/${account.id}" class="btn btn-info">
                                    <i class="glyphicon glyphicon-book"></i>
                                    <spring:message code="show.link.label"/>
                                </a>
                            </td>
                            <td><c:out value='${account.auditor}'/></td>
                            <td><c:out value='${account.code}'/></td>
                            <td><c:out value='${account.title}'/></td>
                            <td><c:out value='${account.nameSecondary}'/></td>
                            <td><c:out value='${account.companyCode}'/></td>
                            <td><c:out value='${account.accountGroup}'/></td>
                            <td><c:out value='${account.accountStatement}'/></td>
                            ...
                            <td>
                                <a href="${pageContext.request.contextPath}/account/delete/${account.id}" class="btn btn-danger" onclick="return confirm('Are you sure to delete?');" >
